...
export function A2UIPanel({ backendUrl }: Props) {
  ...
  return (
    <A2UIProvider catalog={basicCatalog} onAction={handleAction}>
      {lastAction && (
        <div className="mb-2 px-2 py-1 rounded bg-indigo-500/20 border border-indigo-500/40 text-xs text-indigo-300">
          ✓ Action sent: <span className="font-mono">{lastAction}</span>
        </div>
      )}
